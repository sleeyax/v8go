...
func reverseUint8ArrayFunctionCallback(info *FunctionCallbackInfo) *Value {
	...
	args := info.Args()
	if len(args) != 1 {
		return iso.ThrowExceptionStringUnstable("Function ReverseUint8Array expects 1 parameter")
	}
	if !args[0].IsUint8Array() {
		return iso.ThrowExceptionStringUnstable("Function ReverseUint8Array expects Uint8Array parameter")
	}
	...
	if err != nil {
		return iso.ThrowExceptionStringUnstable(fmt.Sprintf("Could not get value for array: %v\n", err))
	}
	return val
}
...
func injectUint8ArrayTester(ctx *Context) error {
	...
	reverseFn := NewFunctionTemplate(iso, reverseUint8ArrayFunctionCallback)
